...
export class CategoryService {
  ...
  protected convertDateFromClient(category: ICategory): ICategory {
    const copy: ICategory = Object.assign({}, category, {
      creationDate: category.creationDate != null && category.creationDate.isValid() ? category.creationDate.format(DATE_FORMAT) : null,
      updateDate: category.updateDate != null && category.updateDate.isValid() ? category.updateDate.format(DATE_FORMAT) : null
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((category: ICategory) => {
        category.creationDate = category.creationDate != null ? moment(category.creationDate) : null;
        category.updateDate = category.updateDate != null ? moment(category.updateDate) : null;
      });
    }
    return res;
  }
